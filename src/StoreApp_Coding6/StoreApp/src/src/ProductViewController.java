...
public class ProductViewController extends JFrame implements ActionListener {
    ...
    private void saveProduct() {
        ...
        try {
        	...
        	{
        		JOptionPane.showMessageDialog(null, "ProductID is required"); // Required Field Validation Added
        	}
            productID = Integer.parseInt(txtProductID.getText());
            if(productID < 0)
            {
            	JOptionPane.showMessageDialog(null, "ProductID cannot be negative"); // Negative Value Validation Added
            }
        }
        ...
        try {
        	...
        	{
        		JOptionPane.showMessageDialog(null, "ProductPrice is required"); // Required Field Validation Added
        	}
            productPrice = Double.parseDouble(txtProductPrice.getText());
            if(productPrice < 0)
            {
            	JOptionPane.showMessageDialog(null, "ProductPrice cannot be negative");// Negative Value Validation Added
            }
        }
        catch (NumberFormatException e) {
        ...
        try {
        	...
        	{
        		JOptionPane.showMessageDialog(null, "ProductQuantity is required"); // Required Field Validation Added
        	}
            productQuantity = Double.parseDouble(txtProductQuantity.getText());
            if(productQuantity < 0)
            {
            	JOptionPane.showMessageDialog(null, "ProductQuantity cannot be negative"); // Negative Value Validation Added
            }
        }
        catch (NumberFormatException e) {
        ...
        // Store the product to the databaseS
